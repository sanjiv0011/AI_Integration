...
public class SetDataIntoTextInputField {
	...
	//[FOR SINLE ENTRY] USE THIS TO FILL THE DATA IN THE INPUT FILED ONLY FOR TEXT FIELD
	...
	//[FOR MULTIPLE ENTRYIES] USE THIS TO FILL THE DATA IN THE INPUT FILED ONLY FOR TEXT FIELD
	public boolean callMeToFillDataIntoTextInputFieldWithNameAndXpathAndValue(WebDriver driver, String[] fieldName, String[] xpathAddress,
			String[] value) throws InterruptedException {
		StackTraceElement stackTraceElement[] = Thread.currentThread().getStackTrace();
		String callerMethodName = stackTraceElement[2].getMethodName();
		...
		while(totalInputField != 0) {
			...
			try {
				logger.info("Element name is >>" + fieldName_Lable + " and address is >> " + xpathAddress_Lable+" and values is >> "+value_Lable);
				...
				logger.info("Value Enteterd in the input field: "+fieldName_Lable+" with iteration count: "+iteratorCount);
				isDataEntered = true;
				Thread.sleep(200);
